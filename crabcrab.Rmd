...
```{r}
# without height
fit <- aov(Age ~ Weight + Diameter + Sex + Length, data=mydata)
summary(fit)
```
```{r}
# without length
fit <- aov(Age ~ Weight + Diameter + Sex + Height, data=mydata)
summary(fit)
```

```{r}
# without diameter
fit <- aov(Age ~ Weight + Length + Sex + Height, data=mydata)
summary(fit)
```

```{r}
# without diameter and height
fit <- aov(Age ~ Weight + Length + Sex, data=mydata)
summary(fit)
```
